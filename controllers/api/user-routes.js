  ...
  try {
    ...
    if (checkuserData) {
      res
        .status(400)
        .json({
          err: "Email already exists. Please sign up with a different email address.",
        });
      return;
    }
    ...
    if (!email || !password || !confirm_password) {
      return res
        .status(400)
        .json({
          errors: ["Email, password, and confirm_password are required"],
        });
    }
    ...
    if (password.length < 8) {
      return res
        .status(400)
        .json({ err: "Password needs to be at least 8 characters" });
    }
